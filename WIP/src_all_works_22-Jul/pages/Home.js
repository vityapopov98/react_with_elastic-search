...
class Home extends Component {
  ...
  onFilterDeleted(filter){
    ...
    ()=>{
        getData(this.state.selectedPage).then(response=>{
            this.gotDataHandle(response.data, response.page*2)
        })
    }
    )
  }
  ...
  filterDataset = ()=>{
    ...
    //------ НОВОЕ -------
    //Запросить в Elastic search данные на основе примененных фильтров
    //Полученные данные сохранить в this.state.fDataset
  ...
  componentDidMount(){
    ...
    gotDataHandle = (data, page) =>{
        ...
        if (data.hits != undefined) {
            ...
            this.setState({fDataset: data.hits.hits, selectedPage: page/2, error: ''}, ()=>{ 
